...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderStaff(g) {
    ...
    window.HERMOSA_STAFF.forEach(s=>{
      const b=document.createElement("button");
      b.type="button"; b.className=`staff-card ${g.staff===s.name?"selected":""}`;
      b.innerHTML=`<span class="staff-avatar">${s.initials}</span><span class="staff-copy"><strong>${s.name}</strong><small>${s.description}</small><em>${s.rating==="Flexible"?"Flexible":"★★★★★ "+s.rating}</em></span><i>✓</i>`;
      b.onclick=()=>{g.staff=s.name;renderGuestEditor();validate();};
      box.appendChild(b);
    });
  }
